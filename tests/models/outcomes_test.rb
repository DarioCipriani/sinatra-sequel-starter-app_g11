...
# prueba de la clase outcome
class OutcomTest < MiniTest::Unit::TestCase
  # Con este test me aseguro que una choise_id y una career_id no estaran duplicadas,
  # este par debe ser unico para outcome para ello en el modelo de outcome puse la restriccion
  # de unique(career_id,choice_id) en este test uso assert_raises: esta función prueba que una
  # llamada a una función en este caso "save" genere una
  # excepción específica (ValidationFailed) cuando se le presenten ciertos parámetros, en este
  # caso una outcome duplicada.
  ...
  def test_outcomes_has_one_choice
    ...
    # seria como assert_equal(falla, duplicate_outcome.save())
    assert_raises(Sequel::ValidationFailed) do
      duplicate_outcome.save
    end
